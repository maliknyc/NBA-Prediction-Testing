...
def run_model_with_target(target_points):
    ...
    jb_cleaned['avg_PTS_5'] = jb_cleaned['PTS'].rolling(window=5).mean()
    jb_cleaned['avg_PTS_10'] = jb_cleaned['PTS'].rolling(window=10).mean()
    jb_cleaned['avg_TPA_5'] = jb_cleaned['TPA'].rolling(window=5).mean()
    jb_cleaned['trend_PTS'] = jb_cleaned['PTS'].rolling(window=5).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0], raw=True)
    jb_cleaned['perc_over'] = jb_cleaned['over_target'].rolling(window=10).mean() * 100
